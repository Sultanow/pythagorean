...
@numba.njit('u8[:, :, :](i8, i8, i8, u8[:, :, :], u4, u1[:], u4[:], u2[:, :, :], u4, u1[:])', cache = True, parallel = True,
    locals = dict(t = numba.int64, s = numba.int64, u = numba.int64, i = numba.int64, tb = numba.float64, tim = numba.float64))
def generateData(cpu_count, L, limit, A, k0, f0, fi0, fc0, k1, f1):
    def is_square(x):
    ...
    while True:
        ...
        block = max(1, (A.shape[0] + nblocks - 1) // nblocks)
        ...
        for iMblock in range(0, nblocks, cpu_count):
            ...
            print(f'l {A.shape[-1] + 1}/{L},', f'i {str(min(A.shape[0], block * min(nblocks, iMblock + cpu_count)) >> 10).rjust(5)}/{A.shape[0] >> 10} K,',
                'ELA', round(tim / 60.0, 1), 'min, ETA', round((nblocks - min(nblocks, iMblock + cpu_count)) * (tim / min(nblocks, iMblock + cpu_count)) / 60.0, 1), 'min')
        print()
...
def load_existing_data():
    import csv
    IsSquare = lambda x: int(math.sqrt(float(x)) + 0.5) ** 2 == x
    A = np.zeros((1 << 10, 2, 3), dtype = np.uint64)
    pos = 0
    # https://github.com/Sultanow/pythagorean/blob/main/pythagorean_12000000.csv?raw=true
    with open('pythagorean_12000000.csv', 'r', encoding = 'utf-8') as f:
        for row in csv.DictReader(f, delimiter = ','):
            if pos >= A.shape[0]:
                A = np.concatenate((A, np.zeros_like(A)), axis = 0)
            A[pos, 0] = (int(row['x']), int(row['y']), int(row['s']))
            A[pos, 1, 0] = A[pos, 0, 0] ** 2
            A[pos, 1, 1] = A[pos, 1, 0] + A[pos, 0, 1] ** 2
            A[pos, 1, 2] = A[pos, 1, 1] + A[pos, 0, 2] ** 2
            
            assert IsSquare(A[pos, 1, 0]), A[pos]
            assert IsSquare(A[pos, 1, 1] - A[pos, 1, 0]), A[pos]
            assert IsSquare(A[pos, 1, 1]), A[pos]
            assert IsSquare(A[pos, 1, 2] - A[pos, 1, 0]), A[pos]
            assert IsSquare(A[pos, 1, 2] - A[pos, 1, 1]), A[pos]
            assert IsSquare(A[pos, 1, 2]), A[pos]
...
def main():
    ...
    A = load_existing_data() if load_csv else np.zeros((0, 2, 0), dtype = np.uint64)
    
    filts = create_filters()
    fc0 = filter_chain_create_load(filts[0][0], filts[0][2])
    
    start = time.time()
    res = generateData(multiprocessing.cpu_count(), L, limit, A,
